...
Instructions:  
Step 1: Modify the code in readOneTestCase().
...
How does this work?  
When you run ./executable, it reads and parses all test cases from stdin, and writes each test case into an individual input file specific to that test case.  
	...
	*/
	...
	if (argc == 1)
	{
		...
		for (int testnum=1;testnum<=number_of_test_cases;testnum++) {
			...
			wait_for_running_children_to_be_under_limit(running_child_pids, MAX_NUM_PROCESSES - 1);
		}
