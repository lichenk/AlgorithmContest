...
void sqrt_modify(int l, int r, int val) {
  ...
  if (lg==rg) {
    ...
    naive_modify(sqrt_ridx[rg],r,val);
...
int main()
{
  ...
  for (int i=0;i<q;i++) {
    ...
    if (s[0]=='c') {
      int ans=0;
      for (int x=l;x<=r;x++) {
        for (auto &w:vlucky) if (w==a[x]) ans++;
      }
      printf("%d\n",ans);
    }
    else {
      int d; cin>>d;
      for (int x=l;x<=r;x++) a[x]+=d;
    }
  }
}