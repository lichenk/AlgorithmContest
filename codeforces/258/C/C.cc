...
int main() 
{
  ...
  for (int i=1;i<mn;i++) pre[i]+=pre[i-1];
  ll final=0;
  for (int x=1;x<mn;x++) {
    ll ans=1;
    int fsz=vf[x].size();
    for (int i=fsz-1;i>=0;i--) {
      int cnt;
      if (i==fsz-1) cnt=gc(vf[x][i],mn-1);
      else cnt=gc(vf[x][i],vf[x][i+1]-1);
      int cand;
      if (vf[x][i]==x) {
        cand=mod_pow(i+1,cnt,MOD)-mod_pow(i,cnt,MOD);
        if (cand<0) cand+=MOD;
      }
      else {
        cand=mod_pow(i+1,cnt,MOD);
      }
      ans=mul(ans,cand);
    }
    final+=ans; final%=MOD;
