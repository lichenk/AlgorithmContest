...
int main() 
{
  ...
  for (int xlo=n-1;xlo>=0;xlo--) {
    ...
    for (int xhi=xlo;xhi<n;xhi++) {
      ...
      for (int yhi=0;yhi<m;yhi++) {
        {
          int value=a[xhi][yhi];
          ylo=max(ylo,fquery(value,yhi));
          fins(value,yhi+1);
          //if(xlo==0&&xhi==0)dbg("Ins. value:%d yhi+1:%d\n",value,yhi+1);
          ylo=max(ylo,vylo[yhi]);
          vylo[yhi]=ylo;
          vans[xlo][xhi][yhi]=ylo;
          //if (xlo==0&&xhi==0&&yhi==3) dbg("ylo:%d\n",ylo);
        }
        {
          int value=a[xlo][yhi];
    ...
    for (int x=xlo;x<n;x++) for (int y=0;y<m;y++) {
      fzero(a[x][y],y+1);
    }
  }
  int final=0;
  for (int xhi=0;xhi<n;xhi++) {
    ...
    for (int xlo=xhi;xlo>=0;xlo--) {
      ...
      for (int yhi=0;yhi<m;yhi++) {
        int value=a[xlo][yhi];
        ylo=max(ylo,(int)vans[xhi][xlo][yhi]);
        //dbg("Get. value:%d ylo:%d\n",value,fquery(value,yhi));
        fins(value,yhi+1);
        //dbg("Ins. value:%d yhi+1:%d\n",value,yhi+1);
        ylo=max(ylo,vylo[yhi]);
        vylo[yhi]=ylo;
        int rylo=max((int)vans[xlo][xhi][yhi],ylo);
        int cand=(xhi-xlo+1)*(yhi-rylo+1);
        //if (xlo==0&&xhi==0&&yhi==3) dbg("ylo:%d yrlo:%d cand:%d\n",ylo,rylo,cand);
        //if (cand==10) dbg("xlo:%d xhi:%d ylo:%d yhi:%d\n",xlo,xhi,ylo,yhi);
        chkmax(final,cand);
      }
    }
  }
