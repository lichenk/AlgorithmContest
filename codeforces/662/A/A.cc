...
bigint bigint_pow(ll inbase, ll e) {
	bigint ans=1;
	bigint base=inbase;
	while(e>0) {
		if (e&1) {
...
int main()
{
	...
	mat.resize(60);
	for (ll j=0;j<60;j++) mat[j].resize(n+1);
	for (ll i=0;i<n;i++) {
	...
	if (freevars==-1) {
		...
		ll fixedvars=n-freevars;
		//printf("fv:%lld\n",fixedvars);
		bigint denom=bigint_pow(2,fixedvars);
		//ll denom=1LL<<fixedvars;
		cout<<denom-1<<"/"<<denom<<endl;
	}
}
