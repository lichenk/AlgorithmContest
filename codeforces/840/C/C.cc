...
const int mn=301;
int S[mn][mn];
int meow(int n, int k) {
  if (S[n][k]!=-1) return S[n][k];
  if (n==0&&k==0) return 1;
  if (n==0||k==0) return 0;
  ll ans=0;
  for (int x=1;x<=n;x++) {
    ans=(ans+binom(n,x)*(ll)mul(meow(n-x,k-1),fact[x]))%MOD;
    //printf("%d %d\n",binom(n,x),meow(n-x,k));
  }
  //retur
  //ll ans=k*(ll)meow(n-1,k) + meow(n-1,k-1);
  return S[n][k]=(ans);
}

int g(int n, int t) {
  int ans=((t&1)?1:-1)*(meow(n,t)*(ll)invfact[t])%MOD;
  if (ans<0) ans+=MOD;
  return ans;
}
...
int solve(int n, vector<int> cliques) {
  ...
  for (int i = 0; i <= n; i++) {
      add(ans, 1LL * p[cur][i] % MOD * fact[i] % MOD);
  }
  if (flip) ans=(MOD-ans)%MOD;
  return (ans+MOD)%MOD;
}
