...
public:
  ...
  MergeRankRollback() {
    for (int x=0;x<size;x++) data.arr[x]={x,1,0};
  }
  pii getRootAndVal(int x) {
    ...
    if (data[x].parent==x) {
      ...
      int resval=got.snd^data[x].val;
      data.modify(x,{got.fst,data[x].rank,resval});
      return MP(got.fst,resval);
    }
  }
  void addEdge(int x, int y, int d, Basis &basis) {
  	//printf("add: %d %d %d\n",x,y,d);
    pii xp=getRootAndVal(x),yp=getRootAndVal(y);
    int xrank=data[xp.fst].rank;
    int yrank=data[yp.fst].rank;
    if (xrank < yrank) {swap(x,y); swap(xp,yp); swap(xrank,yrank);}
    int newrank=(xrank==yrank)?xrank+1:xrank;
    int xr=xp.fst,yr=yp.fst;
    if (xr==yr) {
      basis.add(xp.snd^yp.snd^d);
    }
    else {
      data.modify(xr, {xr, newrank, 0});
      data.modify(yr, {xr, 0, xp.snd^yp.snd^d});
    }
  }
  ...
  void divconq(int l, int r, Basis basis) {
    ...
    if (l+1==r) {
      ...
      for (int qi=l+1;qi<=mid;qi++) {
        ...
        if (oper.type == ADDED) {
          auto &edge=vedge[oper.ei];
          if (!(mid<=edge.rm&&edge.rm<r)) mr.addEdge(edge.x,edge.y,edge.d,childBasis);
        }
...
int main()
{
  ...
  bitset<mn> initialEdges; initialEdges.set();
  for (int i=0;i<m;i++) {
    int x=rint(),y=rint(),d=rint(); if (x>y) swap(x,y);
    int ei=adde(x,y,d,0); assert(ei==i);
  }
