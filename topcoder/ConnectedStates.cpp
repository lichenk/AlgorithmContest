...
class ConnectedStates {
	...
	int getSum(vector <int> _a) {
		...
		ll suma=0;
		for (auto &w:a) suma=add(suma,w);
		ll proda=1;
		for (auto &w:a) proda=mul(proda,w);
		vll poly={1};
		for (auto &c:a) {
			vll v={1,c};
			poly=pmul(poly,v);
		}
		//for (auto &w:poly) printf("%lld ",w);
		//printf(" end poly\n");
		vll ep;
		for (ll k=0;k<=n-2;k++) {
			ll coeff=mod_pow(suma,k,MOD);
			coeff=mul(coeff, invfact[k]);
			ep.PB(coeff);
		}
		//for (auto &w:ep) printf("%lld ",w);
		//printf(" end ep n-2:%lld\n",n-2);
		vll egf=pmul(poly,ep);
		for (auto &w:egf) w=mul(w,proda);
		ll ans=egf[n-2];
		ll final=mul(ans,fact[n-2]);
		return final;
	}
};
// BEGIN CUT HERE
