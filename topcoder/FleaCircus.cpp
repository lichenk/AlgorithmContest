...
template<class T> T mod_inv(T a) { T x, y; extgcd(a, MOD, x, y); return (MOD + x % MOD) % MOD; }
...
void init() {
	...
	for (ll x=0;x<MF;x++) invfact[x]=mod_inv(fact[x]);
}
const ll MN=700;
ll cc[MN];
bool seen[MN];
ll go(ll g, ll k, ll l) {
	ll n=g*k;
	ll groupings=mul(mul(fact[n],mod_inv(mod_pow(fact[k],g))),mod_inv(fact[g]));
	ll part=mul(fact[k-1],mod_pow(l,k-1));
	ll fullpart=mod_pow(part,g);
	return mul(fullpart,groupings);
}
ll getf(ll n, ll l) {
	ll ans=0;
	for (ll fours=0;fours<=n/4;fours++) {
		for (ll twos=0;twos<=(n-fours*4)/2;twos++) {
			ll sum=fours*4+twos*2;
			ll ones=n-sum;
			if (ones<0) break;
			ll choosing=mul(fact[n],mod_inv(mul(fact[fours*4],mul(fact[twos*2],fact[ones*1]))));
			ll cand=mul(go(fours,4,l),mul(go(twos,2,l),go(ones,1,l)));
			ll full=mul(cand,choosing);
			//printf("fours:%lld twos:%lld ones:%lld cand:%lld\n",fours,twos,ones,cand);
			ans=add(ans,full);
		}
	}
	return ans;
