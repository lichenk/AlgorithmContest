...
const ll mp=63;
ll f[2][2][mp];
ll solvehelp(ll ke, ll qe, ll n,ll p, ll m) {
	...
	for (ll i=0;i<n;i++) {
		...
		for (ll k=0;k<2;k++) {
			for (ll x=0;x<=ke;x++) {
				printf("i:%lld k:%lld x:%lld f:%lld\n",i,k,x,f[cur][k][x]);
				for (ll y=ke-m;y<=ke;y++) {
					ll z=min(x+y,ke);
					ll nk=k;
					if (y==ke-m) nk=1;
					f[nxt][nk][z]+=mul(f[cur][k][x],totient(p,ke-y));
					f[nxt][nk][z]%=MOD;
				}
			}
		}
...
ll solve(ll ke, ll qe, ll n,ll p) {
	ll ans=0;
	for (ll m=0;m<=ke;m++) {
		ll oget=solvehelp(ke,qe,n,p,m);
		ll get=mul(oget,mod_pow(totient(p,m),MOD-2,MOD));
		printf("m:%lld oget:%lld get:%lld\n",m,oget,get);
		ans=(ans+get)%MOD;
	}
	return ans;
}
