...
struct rational_t { ll nu, de;
  ...
  rational_t operator/(const ll d) const
  { return rational_t( nu, de*d ); }
...
class OptimalBetting {
	...
	double findProbability(int a, int b, int klim) {
		h[klim].PB(MP(0,rational_t(0,1)));
		h[klim].PB(MP(b,rational_t(1,1)));
		h[klim].PB(MP(INF,rational_t(1,1)));
		for (ll k=klim;k>=1;k--) {
			ll hsz=h[k].size()-1;
			rat prev=rational_t(0,1);
			h[k-1].PB(MP(0,prev));
			for (ll x=1;x<b;x++) {
				rat ans=rational_t(0,1);
				for (ll p=0;p<hsz;p++) for (ll q=p;q<hsz;q++) {
					ll pl=h[k][p].fst,pr=h[k][p+1].fst-1;
					ll ql=h[k][q].fst,qr=h[k][q+1].fst-1;
					rat prob=(h[k][p].snd+h[k][q].snd)/2;
			...
			h[k-1].PB(MP(b,rational_t(1,1)));
			h[k-1].PB(MP(INF,rational_t(1,1)));
		}
